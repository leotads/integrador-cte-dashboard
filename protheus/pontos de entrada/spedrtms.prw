...
User Function SPEDRTMS()
    ...
    If cReg == "D100"
        ...
        /*
        Consultar o conte�do do campo C5_TPFRETE e fazer esse De/Para:
        C = 0
        F = 1
        T = 2
        */
        ...
        If  aCmpAntSFT[43] == "S"
            ...
            vValServ  := LTrim(Transform(F2_VALMERC, "@E 99999999.99"))
        ...
        Else
            ...
            vValServ := LTrim(Transform(F1_VALMERC, "@E 99999999.99"))
        ...
        vValBSICM := LTrim(Transform(F3_BASEICM, "@E 99999999.99"))

        if (SF3->F3_ISENICM > 0 .or. SF3->F3_OUTRICM > 0) .and. SF3->F3_BASEICM == 0
            vValBSICM  := 0 
            vValICMS   := 0
        endif
        ...
        AAdd(vLinha,    IIf(cModelo$"63" .or. !empty(aCmpAntSFT[7]), "", cCod_Part))      //04 COD_PART
        AAdd(vLinha,    AModNot(aCmpAntSFT[42])) //05 COD_MOD
        AAdd(vLinha,    iif(!empty(aCmpAntSFT[7]), "02","00" ) )           //06 COD_SIT
        ...
        AAdd(vLinha,    iif(!empty(aCmpAntSFT[7]), "", 0 ) )              //21 VL_NT
        AAdd(vLinha,    "")             //22 COD_INF
        AAdd(vLinha,    "")             //23 COD_CTA
        AAdd(vLinha,    iif(!empty(aCmpAntSFT[7]), "", cCodMunOri ) )     //24 COD_MUN_ORIG
        AAdd(vLinha,    iif(!empty(aCmpAntSFT[7]), "", cCodMunDes ) )     //25 COD_MUN_DEST
        AAdd(aRet, vLinha)
