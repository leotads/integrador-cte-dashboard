...
User Function LOSF0001(nRecNoZZ1)
    ...
    cEmailSup := Alltrim(SuperGetMv("MS_TIMAIL",.T.,""))

    cQuery := " SELECT "    
    cQuery += " R_E_C_N_O_ AS RECZZ1 FROM " + RetSqlTab('ZZ1')
    cQuery += " WHERE D_E_L_E_T_ = '' "
    
    If nRecNoZZ1 == 0
        cQuery += " AND ZZ1_STATUS = 'A' "
    Else
        cQuery += " AND R_E_C_N_O_ = " + cValToChar(nRecNoZZ1)
    EndIf
    ...
    While QRYZZ1->(!Eof())
        cRet := ProcessaCTE(QRYZZ1->RECZZ1, 'P')        
        
        RecLock('ZZ1', .F.)
            ZZ1->ZZ1_STATUS := Iif(cRet != 'NF Gerada', 'E','P')
            ZZ1->ZZ1_ERRO   := Iif(cRet != 'NF Gerada', cRet,'')
            ZZ1->ZZ1_ERRFIS := Iif(!Empty(cErrorBlock), cErrorBlock, '')
            ZZ1->ZZ1_STAFIS := Iif(!Empty(cErrorBlock), 'E', 'P')
            // ZZ1->ZZ1_SERCTE := cYNumSer
            // ZZ1->ZZ1_NUMCTE := cYNumNF
        ZZ1->(MsUnLock())
        If ZZ1->ZZ1_STATUS == 'E' .AND. !Empty(cEmailSup)
            U_EnviaEmail(" Processamento CTE - Erro", cEmailSup, "Foi encontrada a seguinte msg de erro ( ZZ1_CHAVE = "+ZZ1->ZZ1_CHAVE+") :"+CRLF+Alltrim(ZZ1->ZZ1_ERRO), "", "", .F.)
        EndIf
        QRYZZ1->(DbSkip())
    EndDo
...
Static Function ProcessaCTE(nRecZZ1, cTipo )
    ...
    If !  SX5->(DbSeek(xFilial('SX5') +  '05' + 'CTE'))
        return "Tipo CTE n�o localizado no cadastro de tipo de titulos"
    EndIf
    ...
    Else
        Return "Tag _INFCTE:_REM:_CNPJ n�o encontrada no xml do CTE"
    EndIf
    ...
    Else
        Return "Tag _INFCTE:_DEST:_CNPJ n�o encontrada no xml do CTE"
    EndIf
    ...
    For i := 1 to Len(aVars)
        ...
        If ValType(&(aVars[i][2])) == 'U'
            Return "Tag " + aVars[i][1] + " n�o encontrada no xml do CTE"
        EndIf
    Next
    ...
    If cTomador == 'U'
        Return "Tag Tomador n�o encontrada no xml do CTE"
    EndIf
    ...
    If ! setEmpresa(cCNPJ)
        return 'Cadastro de filial n�o localizado com o CNPJ ' + cCNPJ
    EndIf
    ...
    If ! SA1->(DbSeek( xFilial("SA1") + PADR(cCNPJCli,TamSx3("A1_CGC")[1]) ))
        ...
        ZZ1->(MsUnLock())
        Return 'Cliente(tomador) n�o cadastrado no Protheus ' + cCNPJCli
    Else
        cCODCli := SA1->A1_COD
        cLojCli := SA1->A1_LOJA
        cNomCli := SA1->A1_NOME
    ...
    If ! SA1->(DbSeek( xFilial("SA1") + PADR(cCNPJRem,TamSx3("A1_CGC")[1]) ))
        ...
        If Empty(cAux)
            ...
            cNomRem := SA1->A1_NOME
        Else
            Return "Remetente n�o cadastrado na (SA1)"+CRLF+cAux
        EndIf
    Else
        cCODRem := SA1->A1_COD
        cLojRem := SA1->A1_LOJA
    ...
    If Empty(cTes)
        Return 'TES n�o localizada para o CFOP ' + cCFOP
    EndIf
    ...
    If ! SA1->(DbSeek(xFilial('SA1') + cCnpjDest))
        ...
        If Empty(cAux)
            ...
            cNomDes := SA1->A1_NOME
        Else
            Return "Destinat�rio n�o cadastrado na (SA1)"+CRLF+cAux
        EndIf
    Else
        cCODDes := SA1->A1_COD
        cLojDes := SA1->A1_LOJA
 ...
 * M�todo usado para exclus�o e reprocessamento de dados fiscais da nota.
/*/
...
Static Function ExcluiCTE()
    ...
    If ! setEmpresa(cCNPJ)
        cMensagem := 'Cadastro de filial n�o localizado com o CNPJ ' + cCNPJ
        return 'Cadastro de filial n�o localizado com o CNPJ ' + cCNPJ
    EndIf
    ...
    If lRetorno
        ...
        EndIF
    Else
        cMensagem := "Nota n�o foi inclu�da no Protheus, portanto n�o pode ser cancelada. N�o foi encontrado registro (ZZ1) de inclus�o para esta nota."
    EndIf
    END TRANSACTION
Return cMensagem

/*/{Protheus.doc} GravaSA1
 * Grava destinat�rio, caso necess�rio
/*/
...
/*/{Protheus.doc} VAlida e cria ZZ1 de inclus�o
 * Antes realizar o cancelamento valida a exist�ncia de um registro de
 * inclus�o na tabela ZZ1.
/*/
Static Function VldExist()
    ...
    Local cBody     := ""
    Local cChave    := ""
    //Verifica se existe cadastro de inclus�o de nota 
    //Vindo a partir da intergra��o com o NUCCI
...
Static Function InutCTE() 
    ...
    If ValType(cCNPJ) == "U"
        cMensagem := "XML inv�lido entre em contato o TI NUCCI para resolu��o."+CRLF+" A 'tag_soap_Envelope:_soap_Body:_cteInutilizacaoCTResult:_retInutCTe:_infInut:_CNPJ' n�o est� presente no XML. " 
        return cMensagem
    EndIf
 ...
 * Grava grupo de regi�o na tabela DUY, usado no m�dulo do TMS.
/*/
...
Static Function CancFis()
    ...
    If ValType(cDtcanc) == "U"
        cRetorno := "Data do evento n�o encontrada no XML." + CRLF
    EndIf
    ...
    If SFT->(DbSeek(SF2->F2_FILIAL + 'S' + SF2->(F2_SERIE + F2_DOC + F2_CLIENTE + F2_LOJA)))
        ...
        SFT->(MsUnLock())
    Else
        cRetorno := "SFT n�o encontrada." + CRLF
    EndIf 
    ...
    If SF3->(DbSeek(SF2->(F2_FILIAL + F2_CLIENTE + F2_LOJA + F2_DOC + F2_SERIE)))
        ...
        SF3->(MsUnLock())
    Else
        cRetorno := "SF3 n�o encontrada." + CRLF
    EndIf
    ...
    If SE1->(DbSeek(xFilial("SE1") + SF2->(F2_CLIENTE + F2_LOJA + F2_SERIE + F2_DOC ) ))
        ...
        If lMsErroAuto
            cRetorno := "Error ao cancelar t�tulo (SE1)" + CRLF
            MostraErro(GetSrvProfString("Startpath","") , cArqErro )
            cRetorno += Alltrim(MemoRead( GetSrvProfString("Startpath","") + '\' + cArqErro ))
        EndIf
    Else
        cRetorno := "Error ao cancelar t�tulo (SE1)" + CRLF
        cRetorno += "T�tulo n�o encontrado na filial: " + xFilial('SE1')
    EndIf
