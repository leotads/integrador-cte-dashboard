...
/*/{Protheus.doc} FUNC0005
 * Funcao que processara os xmls e nfse
*/
User Function FUNC0005(nRecNoZA4)
    Local cRet
    Local cEmailSup := ""
    Private cErrorBlock := ''
    Private cYNumSer    := ''
...
Static Function CancFis()
    ...
    If SE1->(DbSeek(xFilial("SE1") + SF2->(F2_CLIENTE + F2_LOJA + F2_SERIE + F2_DOC ) ))
        ...
        If lMsErroAuto
            cRetorno := "Error ao cancelar título (SE1)" + CRLF
            MostraErro(GetSrvProfString("Startpath","") , cArqErro )
            cRetorno += Alltrim(MemoRead( GetSrvProfString("Startpath","") + '\' + cArqErro ))
        EndIf
    Else
        cRetorno := "Error ao cancelar título (SE1)" + CRLF
        cRetorno += "Título não encontrado na filial: " + xFilial('SE1')
    EndIf
